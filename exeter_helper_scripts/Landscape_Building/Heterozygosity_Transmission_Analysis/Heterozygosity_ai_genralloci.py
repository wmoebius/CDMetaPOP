...
def find_loci(columns):
    ...
    The function also checks that both alleles are present for
    every detected locus.
    """
...
def process_run(run_directory, no_heatmaps):
    ...
    # ------------------------------------------------------------------------
    # Detect loci from the first ind file
    # ------------------------------------------------------------------------
    ...
    locus_numbers = find_loci(columns)
    ...
    for generation, file in ind_files:
        ...
        for chunk in pd.read_csv(
            ...
            # ---------------------------------------------------------------
            # Calculate individual heterozygosity across ALL loci
            # ---------------------------------------------------------------
            ...
            for locus in locus_numbers:
                ...
                individual_h += heterozygous.to_numpy(
                    dtype=np.float32
                )

            individual_h /= len(locus_numbers)
