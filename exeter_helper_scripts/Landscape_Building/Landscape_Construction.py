...
if outdir == "":
    #Create a directory name based on the args
        ...
        if args.type == "SLattice":
            outdir = "SLattice_n%d" % (args.n)
    
        if args.type == "HLattice":
            outdir = "HLattice_n%d" % (args.n)
    
        if args.type == "1D":
            outdir = "1D_n%d" % (args.n)
    


        if args.periodic != "False":
            outdir += "_periodic_%s" % (args.periodic)
...
if args.i != "":
    if not os.path.isdir(str(args.i)):
        ...
        os.system("cp "+str(args.i)+"/classvars/ClassVars.csv "+str(outdir)+"/inputs/classvars/ClassVars.csv")
        os.system("cp "+str(args.i)+"/popvars/PopVars.csv "+str(outdir)+"/inputs/popvars/PopVars.csv")
        os.system("cp "+str(args.i)+"/RunVars.csv "+str(outdir)+"/inputs/RunVars.csv")
...
#=============================================================================#
# CONSTUCT DISTANCE MATRIX GIVEN THE TYPE OF LANDSCAPE
#=============================================================================#
if args.type == "RGG":
...
if args.type == "SLattice":
    ...
    ax.set_ylim([-0.5, args.n-0.5])

if args.type == "HLattice":
    ax.set_xlim([-0.5, args.n - 0.5])
    ax.set_ylim([-0.5, args.n * (np.sqrt(3)/2) - 0.5])
